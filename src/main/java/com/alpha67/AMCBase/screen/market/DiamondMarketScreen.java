...
public class DiamondMarketScreen extends ContainerScreen<DiamondMarketContainer> {
    ...
    private final DiamondMarketTile generator;
    ...
    public DiamondMarketScreen(DiamondMarketContainer screenContainer, PlayerInventory inv, ITextComponent titleIn) {
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
      ...
      this.renderBackground(matrixStack);
        ...
        DiamondMarketTile BlockEntity = (DiamondMarketTile) world.getTileEntity(pos);

        double money = this.getContainer().getMoneyContainer();
        double Diamond = this.getContainer().getDiamondPrice();
        double maxPrice = this.getContainer().getMaxPrice();

        int time = this.getContainer().getDiamondTime();


        this.font.drawString(matrixStack, "money : " + money, this.guiLeft + 17, this.guiTop + 40, -16737997);
        this.font.drawString(matrixStack, "diamond price: " + Diamond, this.guiLeft + 17, this.guiTop + 23, -16750900);
        ...
        this.addButton(new Button(this.guiLeft + 119, this.guiTop + 63, 35, 18, name, e -> {
            if (true) {
                AMCBase.PACKET_HANDLER.sendToServer(new ButtonMarket(this.pos, 3));
            }
        }));
